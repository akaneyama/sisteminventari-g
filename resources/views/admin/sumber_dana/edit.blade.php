...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Sumber Dana</h2>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 max-w-2xl">
    <form action="{{ route('sumber-dana.update', $sumber_dana->id_sumber_dana) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" 
                class="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 bg-gray-50 hover:bg-white text-gray-900 sm:text-sm transition duration-200 ease-in-out resize-y">{{ $sumber_dana->deskripsi }}</textarea>
        </div>
