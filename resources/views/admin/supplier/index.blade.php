...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Daftar Supplier</h2>
        <p class="text-sm text-gray-500 mt-1">Kelola data supplier untuk melacak asal-usul barang.</p>
    </div>
    <a href="{{ route('supplier.create') }}" class="inline-flex items-center justify-center py-2.5 px-5 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 ease-in-out transform hover:-translate-y-0.5 w-full sm:w-auto">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4">
    <form action="{{ route('supplier.index') }}" method="GET" class="flex gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="{{ $filter['search'] ?? '' }}" placeholder="Cari nama supplier, kontak, telepon, atau email..."
        ...
        @if(!empty($filter['search']))
            <a href="{{ route('supplier.index') }}" class="inline-flex items-center py-2.5 px-4 rounded-xl text-sm font-medium text-red-600 bg-red-50 border border-red-200 hover:bg-red-100">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            <a href="{{ route('supplier.edit', $item->id_supplier) }}" class="p-2 text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors tooltip" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </a>
                            <form action="{{ route('supplier.destroy', $item->id_supplier) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus supplier ini?');">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                        ...
                        <p class="text-base font-medium text-gray-600">Belum ada data supplier.</p>
                        <p class="text-sm mt-1">Silakan tambah supplier baru untuk memulai.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
