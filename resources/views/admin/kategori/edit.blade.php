...
@section('content')
<div class="mb-6">
    <a href="{{ route('kategori.index') }}" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors mb-2">
    ...
    <form action="{{ route('kategori.update', $kategori->id_kategori) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" 
                class="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 bg-gray-50 hover:bg-white text-gray-900 sm:text-sm transition duration-200 ease-in-out resize-y">{{ $kategori->deskripsi }}</textarea>
        </div>
